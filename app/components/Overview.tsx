...
export default function Overview({ selectedGame, games }: any) {
  ...
  if (!metrics || metrics.status === "empty") {
    return (
  ...
  const activeData = isGlobal ? metrics.overall : metrics.games?.[selectedGame] || {};
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <MetricCard
          ...
          info="Total public reviews fetched from Google Play across all tracked games in SQLite."
          value={activeData.ingested?.toLocaleString() || "0"}
          subtitle={
            isGlobal
              ? `Across ${Object.keys(games).length} Games (100%)`
              : `${Math.round((activeData.ingested / (metrics.overall?.ingested || 1)) * 100)}% of Market Sample`
          }
          relativeBadge={
            isGlobal ? (
              ...
              <span className="text-[0.65rem] text-indigo-300 font-mono">
                {activeData.ingested} / {metrics.overall?.ingested}
              </span>
            )
          }
            ...
            isGlobal ? (
              <span
                className={`inline-flex items-center gap-0.5 text-[0.65rem] font-bold px-1.5 py-0.5 rounded ${
                  rel.hw_rating_delta >= 0
                    ? "text-emerald-400 bg-emerald-500/10 border border-emerald-500/20"
                    : "text-rose-400 bg-rose-500/10 border border-rose-500/20"
                }`}
              >
                {rel.hw_rating_delta >= 0 ? <ArrowUpRight size={11} /> : <ArrowDownRight size={11} />}
                <span>
                  Hitwicket {rel.hw_rating_delta >= 0 ? `+${rel.hw_rating_delta}` : rel.hw_rating_delta} vs Rivals
                </span>
              </span>
            ) : (
              <span
                className={`inline-flex items-center gap-0.5 text-[0.65rem] font-bold px-1.5 py-0.5 rounded ${
                  activeData.vs_market_rating >= 0
                    ? "text-emerald-400 bg-emerald-500/10"
                    : "text-rose-400 bg-rose-500/10"
                }`}
              >
                {activeData.vs_market_rating >= 0 ? `+${activeData.vs_market_rating}` : activeData.vs_market_rating} vs Market
              </span>
            )
          }
        ...
        <MetricCard
          ...
          relativeBadge={
            isGlobal ? (
              ...
              >
                {rel.hw_neg_delta <= 0 ? <ArrowDownRight size={11} /> : <ArrowUpRight size={11} />}
                <span>
                  Hitwicket {rel.hw_neg_delta <= 0 ? `${rel.hw_neg_delta}% (Lower)` : `+${rel.hw_neg_delta}%`}
                </span>
              </span>
            ) : (
              <span
                className={`inline-flex items-center gap-0.5 text-[0.65rem] font-bold px-1.5 py-0.5 rounded ${
                  activeData.vs_market_neg <= 0
                    ? "text-emerald-400 bg-emerald-500/10"
                    : "text-rose-400 bg-rose-500/10"
                }`}
              >
                {activeData.vs_market_neg <= 0 ? `${activeData.vs_market_neg}%` : `+${activeData.vs_market_neg}%`} vs Market
              </span>
            )
          }
        ...
        <MetricCard
          ...
          relativeBadge={
            isGlobal ? (
              ...
              >
                {rel.hw_pos_delta >= 0 ? <ArrowUpRight size={11} /> : <ArrowDownRight size={11} />}
                <span>
                  Hitwicket {rel.hw_pos_delta >= 0 ? `+${rel.hw_pos_delta}% (Higher)` : `${rel.hw_pos_delta}%`}
                </span>
              </span>
            ) : (
              <span
                className={`inline-flex items-center gap-0.5 text-[0.65rem] font-bold px-1.5 py-0.5 rounded ${
                  activeData.vs_market_pos >= 0
                    ? "text-emerald-400 bg-emerald-500/10"
                    : "text-rose-400 bg-rose-500/10"
                }`}
              >
                {activeData.vs_market_pos >= 0 ? `+${activeData.vs_market_pos}%` : `${activeData.vs_market_pos}%`} vs Market
              </span>
            )
          }
        ...
        <MetricCard
          ...
          subtitle={
            isGlobal
              ? `Avg ~${Math.round((activeData.classified || 0) / 3)} / Game`
